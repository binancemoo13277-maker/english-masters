...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.status(200).json(rows[0] || { content: null });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (!content || typeof content !== 'object') return res.status(400).json({ error: 'بيانات غير صحيحة' });
      await sql`UPDATE site_content SET content = ${JSON.stringify(content)}::jsonb, updated_at = now() WHERE id = 1`;
